...
def sample_pdf(
    ...
    weights = weights + 1e-5                          # avoid NaN
    pdf = weights / weights.sum(dim=-1, keepdim=True)
    cdf = torch.cumsum(pdf, dim=-1)
    cdf = torch.cat([torch.zeros_like(cdf[:, :1]), cdf], dim=-1)  # (B, n_coarse-1)
...
def volume_render(
    sigma: torch.Tensor,     # (B, S)   density
    rgb: torch.Tensor,       # (B, S, 3) colour
    dust_prob: torch.Tensor, # (B, S)   dust probability
    z: torch.Tensor,         # (B, S)   t-values
    rays_d: torch.Tensor,    # (B, 3)   ray directions
    ...
    # scale by ray-direction magnitude (not needed for unit dirs, but safe)
    ...
    depth_map = (weights * z).sum(dim=1)                       # (B,)
    acc_map   = weights.sum(dim=1)                             # (B,)
    dust_map  = (weights * dust_prob).sum(dim=1)               # (B,)
...
def render_rays(
    ...
    # ---- Coarse pass ----
    pts_c, z_c = sample_stratified(rays_o, rays_d, near, far, n_coarse, perturb=perturb)
    pts_c_flat = pts_c.reshape(-1, 3)
    dirs_c_flat = rays_d[:, None, :].expand_as(pts_c).reshape(-1, 3)

    sigma_c, rgb_c, dust_c = model_coarse(pts_c_flat, dirs_c_flat)
    ...
    z_fine = sample_pdf(z_mids, weights_c[:, 1:-1].detach(), n_fine, det=(not perturb))
    z_fine = z_fine.detach()

    z_f, _ = torch.sort(torch.cat([z_c, z_fine], dim=-1), dim=-1)  # (B, n_coarse+n_fine)
    n_total = z_f.shape[1]
    ...
    sigma_f, rgb_f, dust_f = model_fine(pts_f_flat, dirs_f_flat)
