...
* Temporal dust conditioning via per-frame embeddings
...
from .dataset import DustDataset, get_camera_entries, load_config
...
def photometric_loss(
    ...
    """Weighted MSE.  Pixels with high dust weight contribute more."""
    w   = 1.0 + dust_weight_alpha * weight
    mse = ((pred_rgb - gt_rgb) ** 2).mean(dim=-1)
    return (w * mse).mean()


def dust_regularisation(dust_map: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    """Encourage dust_map to be high where dust_weight is high."""
...
def save_checkpoint(out_dir, step, coarse, fine, optimizer, scheduler, n_frames):
    ckpt = {
        "step":      step,
        "n_frames":  n_frames,
...
def load_checkpoint(path, coarse, fine, optimizer, scheduler, device):
...
def train(args):
    ...
    print(f"[train] n_frames={n_frames}  time_dim={time_dim}")
    ...
    coarse = DustNeRF(**model_kwargs).to(device)
    fine   = DustNeRF(**model_kwargs).to(device)
    ...
    if args.resume and latest_ckpt.exists():
        start_step = load_checkpoint(str(latest_ckpt), coarse, fine,
    ...
    while step < max_steps:
        ...
        frame_idx = batch["frame_idx"].to(device)  # (B,) int64
        ...
        if step % save_every == 0 or step == max_steps:
            save_checkpoint(str(ckpt_dir), step,
                            coarse, fine, optimizer, scheduler, n_frames)
